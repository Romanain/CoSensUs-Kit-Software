...
String labels[] = {"AccX", "AccY", "AccZ", "GyrX", "GyrY", "GyrZ"};
...
// Check if samples are collected
    ...
    if (sampleBufferGyrZ.size() >= SAMPLES)
...
// Gather data for message
      ...
      {
        doc["AccX"][i] = sampleBufferAccX.shift();
        doc["AccY"][i] = sampleBufferAccY.shift();
        doc["AccZ"][i] = sampleBufferAccZ.shift();
        doc["GyrX"][i] = sampleBufferGyrX.shift();
        doc["GyrY"][i] = sampleBufferGyrY.shift();
        doc["GyrZ"][i] = sampleBufferGyrZ.shift();
      }
#endif
